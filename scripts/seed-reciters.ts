/**
 * Seed script: Reciters + Surahs
 *
 * Firestore paths:
 *   reciters/{reciterId}                  — reciter document
 *   reciters/{reciterId}/surahs/{001..N}  — surahs sub-collection
 *
 * Usage:
...
const RECITER_ID = "idris_abkar";
const AUDIODIRNAME = "idris_abkar";
const IMAGEPATH = "idris_abkar";

interface ReciterInput {
  id: string;
  name_en: string;
  name_ar: string;
  image_path: string;
  ...
  surah_count?: number; // defaults to 114
  audioDirName: string; // Storage folder name, e.g. "aimanswaid"
}

// ---------------------------------------------------------------------------
// Reciter data — add more reciters here as needed
// ---------------------------------------------------------------------------

const reciters: ReciterInput[] = [
  {
    id: RECITER_ID,
    name_en: "Idris Abkar",
    name_ar: "إدريس أبكر",
    image_path: `avatars/${IMAGEPATH}.webp`,
    desc: "",
    popularity_score: 0,
    surah_count: 114,
...
async function seedReciters(): Promise<void> {
  const recitersCol = db.collection("reciters");

  console.log(`\nSeeding ${reciters.length} reciter(s)…\n`);

  for (const reciter of reciters) {
    const reciterRef = recitersCol.doc(reciter.id);
    const surahCount = reciter.surah_count ?? 114;
    ...
    const reciterData = {
      ...
      is_active: true,
      desc: reciter.desc ?? "",
      popularity_score: reciter.popularity_score ?? 0,
      play_count: 0,
    ...
    await reciterRef.set(reciterData);
    console.log(`  ✓  reciter: ${reciter.id}  (${reciter.name_en})`);

    const surahsCol = reciterRef.collection("surahs");

    for (let i = 1; i <= surahCount; i++) {
      const surahId = String(i).padStart(3, "0");
      const meta = surahMetadata[i - 1];

      const surahData = {
        id: surahId,
        surah_number: i,
        audio_path: `audio/${reciter.audioDirName}/${surahId}.mp3`,
        image_path: "",
        name_en: meta.englishName,
        name_ar: meta.arabicName,
        transliteration: meta.transliteration,
      };

      await surahsCol.doc(surahId).set(surahData);
      console.log(`      ✓  surah: ${surahId}  (${meta.transliteration})`);
    }
  }

  console.log("\nDone. Structure: reciters/{reciterId}/surahs/{001…114}\n");
}

seedReciters().catch((err) => {
